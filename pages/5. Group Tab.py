...
with col4:
    file_mbuy = st.file_uploader("4. Margin Buy (.txt)", type=['txt'], key='shared_mbuy')
with col5:
    file_sell = st.file_uploader("5. Margin Sell (.txt)", type=['txt'], key='shared_sell')

shared_ready = all([file_cp, file_rp, file_op, file_mbuy, file_sell])

if shared_ready:
    # Hanya parse ulang kalau file berubah (pakai nama file sebagai proxy)
    file_sig = f"{file_cp.name}_{file_rp.name}_{file_op.name}_{file_mbuy.name}_{file_sell.name}"
    if st.session_state.get('_shared_file_sig') != file_sig:
        with st.spinner("⚙️ Memuat file bersama..."):
            ...
            mb_content = file_mbuy.read().decode("utf-8", errors="replace")
            st.session_state['shared_margin_buy']     = parse_margin_buy(mb_content)
            sr_content = file_sell.read().decode("utf-8", errors="replace")
            st.session_state['shared_sell_regular']   = parse_sell_regular(sr_content)
...
with tab_pei:
    ...
    with pc1:
        file_invoice    = st.file_uploader("A. Netting Invoice (.xlsx)", type=['xlsx'], key='pei_invoice')
    with pc2:
        file_sid_client = st.file_uploader("B. SID Client (.xlsx)",      type=['xlsx'], key='pei_sid')

    pei_ready = shared_ready and file_invoice and file_sid_client
    ...
    if pei_ready:
        if st.button("▶ Proses TRX PEI", type="primary", use_container_width=True, key='btn_pei'):
            try:
                with st.spinner("⚙️ Memproses TRX PEI..."):
                    ...
                    margin_buy     = st.session_state['shared_margin_buy']
                    sell_regular   = st.session_state['shared_sell_regular']
                    ...
                    # Konversi sell_regular & margin_buy (dari shared — format {sid: {stock: {lot, value}}})
                    sell_reg_by_sid = sell_regular
                    buy_reg_by_sid  = margin_buy
        ...
        st.info("⬆️ Lengkapi file bersama + Netting Invoice + SID Client untuk memproses TRX PEI.")
